...
class HospitalDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $theatreDepartment = Department::where('code', 'THR')->first();
        ...
        $doctorAccounts = [
            ...
            [
                ...
                'phone' => '+2348000000007',
                'department_id' => $theatreDepartment?->id ?? $clinicalDepartment?->id,
                'password' => 'doctor123',
            ],
        ];

        foreach ($doctorAccounts as $doctorAccount) {
            User::updateOrCreate(
                ['email' => $doctorAccount['email']],
                [
                    'name' => $doctorAccount['name'],
                    'employee_id' => $doctorAccount['employee_id'],
                    'phone' => $doctorAccount['phone'],
                    'role' => 'doctor',
                    'department_id' => $doctorAccount['department_id'],
                    'status' => 'active',
                    'password' => Hash::make($doctorAccount['password']),
                ],
            );
        }

        $doctor = User::where('email', 'sarah.johnson@akobi.com')->first()
            ?? User::where('email', 'doctor@akobi.test')->firstOrFail();
