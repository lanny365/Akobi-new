...
export interface BackendVisit {
  ...
  routed_to_doctor_at?: string | null;
  queued_at?: string | null;
  patient: BackendPatient;
  department?: BackendDepartment | null;
  doctor?: BackendUser | null;
  vital_sign?: BackendVitalSign | null;
}
...
export interface RouteVisitToDoctorPayload {
  doctor_id: number;
  department_id?: number;
  consulting_room?: string;
  route_notes?: string;
  routed_by?: string;
  routed_to_doctor_at?: string;
}

export interface UpdateVisitStatusPayload {
  status: 'queued' | 'vitals-recorded' | 'with-doctor' | 'in-progress' | 'completed' | 'cancelled';
  seen_at?: string;
}
...
export async function fetchDoctorQueue(doctorId?: number): Promise<BackendVisit[]> {
  const query = doctorId ? `?doctor_id=${doctorId}` : '';
  const response = await apiRequest<{ data: BackendVisit[] }>(`/queue/doctors${query}`);
  return response.data;
}

export async function fetchDoctors(): Promise<BackendUser[]> {
  const response = await apiRequest<{ data: BackendUser[] }>('/users?role=doctor&status=active');
  return response.data;
}

export async function recordVisitVitalSigns(
  visitId: number | string,
  payload: RecordVitalSignsPayload,
): Promise<BackendVisit> {
  const response = await apiRequest<{ message: string; data: BackendVisit }>(
...
export async function routeVisitToDoctor(
  visitId: number | string,
  payload: RouteVisitToDoctorPayload,
): Promise<BackendVisit> {
  const response = await apiRequest<{ message: string; data: BackendVisit }>(
    `/visits/${visitId}/route-to-doctor`,
    {
      method: 'PUT',
      body: JSON.stringify(payload),
    },
