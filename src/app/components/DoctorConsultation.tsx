...
export function DoctorConsultation() {
  ...
  const { submitDoctorBilling } = useCashier();
  ...
  const consultations: Consultation[] = patientQueue
    .filter(patient => patient.assignedDoctor === currentDoctor.fullName)
    .map(patient => ({
      id: patient.id,
              ...
              {filteredConsultations.map((consultation) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    // Automatically remove patient from queue when selected
                    removeFromQueue(consultation.patientId);
                    toast.success(`${consultation.patientName} started consultation - removed from queue`);
                    // Reset form
                    setClinicalNotes('');
                    setDiagnosis('');
                    setTreatmentPlan('');
